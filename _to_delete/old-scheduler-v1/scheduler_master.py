...
the rest of the night's work, and its traceback lands in its own log file
rather than in the middle of everyone else's.
"""
...
# The coordinator's own log: which tasks got a turn and how each ended. What a
# task actually DID goes to that task's file - see scheduler/logging_setup.py.
...
def run_due_tasks(db: Session, only_task: Optional[str] = None) -> dict:
    ...
    """
    cfg = scheduler_settings.get_all(db, logger)
