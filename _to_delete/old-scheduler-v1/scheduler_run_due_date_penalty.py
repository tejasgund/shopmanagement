#!/usr/bin/env python3
"""
scheduler/run_due_date_penalty.py - run ONLY the Due Date Penalty task.
...
are therefore the same thing; only the schedule differs.
    ...
    python -m scheduler.run_due_date_penalty --date 2026-08-01     # recompute as of that day

Writes to logs/due_date_penalty.log (and errors.log on failure).
...
TASK_NAME = "due_date_penalty"


def main(argv=None) -> int:
    parser = argparse.ArgumentParser(
        prog="run_due_date_penalty.py",
        description="Run the Due Date Penalty scheduler task and exit.",
    )
