...
no second list anywhere, in the frontend or otherwise, to keep in step. It
also gets its own log file the first time it runs, with no wiring needed.
"""
...
from scheduler import config, errors
from scheduler.logging_setup import get_logger
from scheduler.models import SchedulerTask

# Coordination-level events (bootstrapping, registry problems) belong to the
# coordinator's log. A specific task's failure is written to THAT task's log -
# see execute() below.
...
def now_local() -> datetime:
    """
    Naive local time in the scheduler's configured timezone.

    Naive on purpose: it has to compare against timestamps the application
    stores naive. The timezone comes from scheduler.conf rather than from the
    app - see the README on keeping the two in step.
    """
...
def fail(db: Session, row: SchedulerTask, exc: BaseException) -> SchedulerTask:
    ...
    The traceback goes to THIS task's own log file (and, at ERROR level, to the
    shared errors.log) as well as into the row, so the failure is readable
    ...
    errors.log_exception(
        get_logger(row.task_name), f"{row.task_name} for {row.run_date} FAILED", exc,
    )
    return _finish(db, row, FAILED, error_message=errors.detail(exc))
...
def execute(db: Session, row: SchedulerTask, cfg: dict) -> SchedulerTask:
    ...
    if not task_enabled(cfg, row.task_name):
        get_logger(row.task_name).info(
            "Skipping %s for %s: task is disabled in Scheduler settings",
    ...
    task_log = get_logger(row.task_name)
    try:
        runner = _resolve(TASKS[row.task_name]["runner"])
        task_log.info("Starting %s for %s (attempt %s)",
                      row.task_name, row.run_date, row.attempts)
        result = runner(db, row.run_date, cfg) or {}
        finished = complete(db, row, result)
        task_log.info(
            "Finished %s for %s: %s processed, %s failed, %sms",
            row.task_name, row.run_date, finished.records_processed,
            finished.records_failed, finished.duration_ms,
        )
        return finished
