"""
scheduler/billing/rent.py - automatic monthly rent bill generation.

Owned by the scheduler. The application imports this for the admin's manual
POST /api/bills/generate-rent, so the nightly run and the button execute the
same function and cannot drift into disagreeing about what a rent bill is.

Nothing here imports FastAPI or the application, so this process can run it
with the app's source nowhere on the box.

Deliberately session-driven: every entry point takes the `db` session to work
in, and the generation lock derives its own connection from that session's
engine rather than a module-level one, so the API process and a standalone
cron process can each supply their own database setup.
"""
...
from scheduler.audit import write_audit
from scheduler.errors import TaskBusy
from scheduler.logging_setup import get_logger
from scheduler.models import Bill, Shop, User, UserShop
from scheduler.money import decimal_to_float

logger = get_logger("rent_generation")
...
def _rent_generation_lock(db: Session, timeout_seconds: int = 30):
    ...
    Two layers, because there are two ways to race:
      ...
      - other processes          -> a MySQL named lock, held by the database

    Why this is needed: the API is served by more than one uvicorn worker, and
    the admin's manual trigger can land on any of them at the same moment the
    cron job is running. Without a lock each reads "no rent bill exists yet"
    ...
    try:
        # ── Layer 2: other processes, via the database ──
        if db.get_bind().dialect.name == "mysql":
            ...
            if not db_lock_acquired:
                # Another process is generating right now. Its run covers the
...
def generate_rent_bills_for_date_locked(db: Session, target_date: date) -> dict:
    ...
    the scheduler task and the manual admin endpoint alike.
    """
    try:
...
def generate_rent_bills_for_date(db: Session, target_date: date) -> dict:
    ...
    rent_bill_date matches, since the admin has not opted them in. A user who
    is opted in but currently has no shops assigned produces no bills
    (nothing to bill) - reflected in skipped_no_shops.

    Idempotent: safe to call more than once for the same date (a manual re-run,
    a retry, a backfill) - a user/shop that already has a Rent bill for that
    month is skipped, never duplicated.
    ...
    got two identical rent bills on 2026-08-13. Always call this through
    ...
    # Read once per run, not once per bill: every bill generated by one run
    # should use the same due period even if an admin edits it mid-run.
    due_days = scheduler_settings.get(db, "bill.due_days", logger)
    ...
    for user in users:
        ...
        for user_shop in user_shops:
            ...
            amount_value = decimal_to_float(shop.shop_rent)
            if amount_value <= 0:
            ...
            # Due Date = Bill Date + the admin-configured due period.
            ...
            write_audit(
                db, "AUTO_GENERATE", "bills", bill.id,
                new_data={"user_id": user.id, "shop_id": shop.id,
                          "amount": float(amount), "bill_date": target_dt.isoformat()},
            )
