...
amount, due date, today, settings) and nothing else. An incrementing
...
def quote(bill: Bill, cfg: dict, as_of: date) -> dict:
    """
...
def apply_penalties_for_date(db: Session, target_date: date,
    ...
    """
    cfg = cfg if cfg is not None else settings_service.get_all(db)
    rules = penalty_settings(cfg)
    ...
    for bill in eligible_bills(db):
        ...
        try:
            ...
            old_penalty = Decimal(str(_decimal_to_float(bill.penalty_amount)))
